...
fmm =glm(log(Masa.Promedio)~ factor(Categoria.de.Dieta), data =mdfun, family = Gamma)
anova1 <- aov(fmm)
summary (anova1) # Si hay Diferencias entre categoraias de dieta en cuanto a masa corporal
# Entre cuales Categorias hay las diferencias?
...
fmm2 =glm(log(Masa.Promedio)~ factor(Estrato), data =mdfun, family = Gamma)
anova2 <- aov(fmm2)
summary (anova2) # Si hay Diferencias entre categoraias de dieta en cuanto a masa corporal
# Entre cuales Categorias hay las diferencias?
intervals2 = TukeyHSD(anova2)
par(las=1)
par(mar=c(5,13,4,2))
plot(intervals2)
...
#diferencia entre agrupamientos?
envfit(gua.mds1,data.scores1[4])#la columna 4 de data.scores1 refleja los agrupamientos
# Usando GGPLOT ###
...
###### Graficar NMDS version vieja - NO usada #####
